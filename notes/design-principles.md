...
以上五条已由 Step 2 的能力层实现，并由 `tests/capability/` 的可执行用例验证：`mount()` 只登记意图，依赖不齐时 setup 从不执行；目标视图记录提供者实例身份，暴露相等值的不同实例仍判为替换；提供者被请求离开时立刻退出可解析集合，绑定保留到依赖者停用结算之后才撤回；依赖不满足、激活失败与停用失败是三种不同状态。执行证据见 [Step 2 计划](step2.md)。

### 已决定的假设

- `ServiceKey<T>` 采用对象身份加 `unique symbol` 品牌，不使用字符串解析身份，也不建立进程全局名字表；同名不同键由每个注册表在接收声明时报告冲突。
- 可选依赖不进入第一版响应式依赖图；首个真实用例出现时再连同其响应式语义一起设计。
