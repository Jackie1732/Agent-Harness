...
export function decodeStepDecided(value: unknown, version: 1 | 2 | 3 = 1): AgentEventPayloads['step-decided'] {
  ...
  array(input.actions, 64).forEach(value => {
    ...
    choice(action.route, ['tool', 'send', 'reply', 'wait', 'ask', 'invalid', ...(version !== 1 ? ['spawn', 'await-subagent', 'answer-subagent', 'ask-parent', 'progress'] : []), ...(version === 3 ? ['work-progress', 'work-ask', 'work-receive', 'work-answer'] : [])])
  })
