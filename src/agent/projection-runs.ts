...
export function applyTurnStarted(state: AgentProjectionState, event: CommittedSessionEvent<AgentEventPayloads['turn-started']>): void {
  ...
  if (payload.root === null) {
    if (payload.predecessor !== null || input.status !== 'queued' || input.input?.kind === 'answer' || input.workMessage !== undefined || input.workGroupResult !== undefined) invalidAgent('root-input-not-queued')
