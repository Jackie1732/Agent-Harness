...
export function assertWorkflowMessageFits(sessionId: SessionId, command: MessageSendCommand, limits: WorkflowMessageBudget): void {
  if (command.kind !== 'send') throw new WorkflowError('WORKFLOW_HISTORY_INVALID', 'workflow-command-must-send')
  ...
  const envelope = { envelopeVersion: 1, messageId: id, sender: formatSessionAddress(sessionId),
    recipient: command.request.recipient, channelId: command.request.channelId,
