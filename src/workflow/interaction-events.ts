...
export const workflowInteractionAdmittedEvent = createDurableEventDefinition({ type: 'workflow/interaction-admitted', payloadVersion: 1, ignorable: false,
  decode(value) {
    const p = record(value); exact(p, ['kind', 'definition', 'assignment', 'request', 'targetAssignment', 'observedAt', 'deadline'])
    return { kind: choice(p.kind, ['question'] as const), definition: eventId(p.definition), assignment: workflowReference(p.assignment),
      request: workflowReference(p.request), targetAssignment: workflowReference(p.targetAssignment), observedAt: timestamp(p.observedAt), deadline: timestamp(p.deadline) }
  } })
...
export const workInteractionResolvedEvent = createDurableEventDefinition({ type: 'work/interaction-resolved', payloadVersion: 1, ignorable: false,
  decode(value) {
    ...
    if (outcome === 'admitted') {
      ...
      return { request: eventId(p.request), outcome, admission: workflowReference(p.admission), value: workflowInteractionAdmittedEvent.decode(p.value!) }
    }
...
export const workflowInteractionSettledEvent = createDurableEventDefinition({ type: 'workflow/interaction-settled', payloadVersion: 1, ignorable: false,
  decode(value) {
    ...
    return { interaction: eventId(p.interaction), outcome: choice(p.outcome, ['answered', 'declined', 'timed-out', 'cancelled', 'interrupted']), source: workflowReference(p.source) }
  } })
...
export const workProtocolClassifiedEvent = createDurableEventDefinition({ type: 'work/protocol-classified', payloadVersion: 1, ignorable: false,
  decode(value) {
    ...
    return { accepted: eventId(p.accepted), inbox: eventId(p.inbox), kind: choice(p.kind, ['question', 'answer'] as const),
