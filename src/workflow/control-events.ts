...
export type WorkflowControlRequested = { readonly definition: SessionEventId; readonly requestKey: string;
  readonly kind: 'pause' | 'resume'; readonly reason: string }
export type WorkflowControlSettled = { readonly request: SessionEventId; readonly outcome: 'applied'; readonly owner: string }
export const workflowControlRequestedEvent = createDurableEventDefinition<WorkflowControlRequested & JsonObject>({
  ...
  decode(value) {
    ...
    return { definition: eventId(p.definition), requestKey: text(p.requestKey, 128), kind: choice(p.kind, ['pause', 'resume']), reason: text(p.reason, 1024, true) }
  },
})
export const workflowControlSettledEvent = createDurableEventDefinition<WorkflowControlSettled & JsonObject>({
  ...
  decode(value) {
    ...
    return { request: eventId(p.request), outcome: choice(p.outcome, ['applied']), owner: text(p.owner, 128) }
  },
})
