...
export type HostSubagentConfigV3 = { readonly kind: 'disabled' }
  | Omit<Extract<HostSubagentConfig, { kind: 'enabled' }>, 'workspaceResources'>

/** The v3 Host resource directory is shared by work and child execution. */
export function decodeHostWorkspaceResources(value: unknown): readonly HostWorkspaceResource[] {
  const workspaceResources = array(value, 64).map(value => {
...
export function decodeHostSubagents(value: unknown, sharedResources?: readonly HostWorkspaceResource[]): HostSubagentConfig {
  const input = record(agentJson(value))
  if (input.kind === 'disabled') { exact(input, ['kind']); return { kind: 'disabled' } }
  choice(input.kind, ['enabled']); exact(input, ['kind', 'templates', 'parents',
    ...(sharedResources === undefined ? ['workspaceResources'] : []), 'limits'])
  ...
  const workspaceResources = sharedResources ?? decodeHostWorkspaceResources(input.workspaceResources)
...
export function parentSubagentRole(config: HostSubagentConfig | HostSubagentConfigV3 | undefined, agentKey: string): AgentSubagentRole {
