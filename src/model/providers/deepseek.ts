...
const deepSeekProtocol = {
  ...
  support: { text: true, instructions: true, tools: true, controls: ['temperature'] as const,
    profiles: [`${DEEPSEEK_PROFILE}@1`], continuations: [`${DEEPSEEK_CONTINUATION}@1`] },
  semanticHeaders: {}, authentication: (key: string) => ({ authorization: `Bearer ${key}` }),
  encode: encodeDeepSeekRequest, decode: decodeDeepSeekStream,
}

export function deepSeekModelDescriptor(options: Omit<HttpModelProviderOptions, 'apiKey'>) {
  return httpModelDescriptor(options, deepSeekProtocol)
}
...
export function createDeepSeekModelProvider(options: HttpModelProviderOptions): ModelProvider {
  return new HttpModelProvider(options, deepSeekProtocol)
}
